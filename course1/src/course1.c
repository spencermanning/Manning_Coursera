...
int8_t test_memmove1() {
  ...
  set = (uint8_t*) reserve_words( MEM_SET_SIZE_W ); // MEM_SET_SIZE_W = 10
  if (! set ) 
  {
    return TEST_ERROR;
  }
  ...
  for( i = 0; i < MEM_SET_SIZE_B; i++) // MEM_SET_SIZE_B = 32
  {
    set[i] = i;
  }

  print_array(set, MEM_SET_SIZE_B);
  my_memmove(ptra, ptrb, TEST_MEMMOVE_LENGTH); // TEST_MEMMOVE_LENGTH = 16
  print_array(set, MEM_SET_SIZE_B);
